...
class SplitComparesTransform : public ModulePass {
 ...
 private:
  ...
  /// split solely for the purpose of inserting more edges into CFG (for cov
  /// feedback)
  bool splitForFeedback(CmpInst *cmp_inst, Module &M);
...
bool SplitComparesTransform::splitForFeedback(CmpInst *cmp_inst, Module &M) {
  LLVMContext &C = M.getContext();

  BasicBlock *bb = cmp_inst->getParent();

  // the comparison already has the target bitwidth so we do not split the
  // comparison. However, we will still attempt to split the BasicBlock, s.t.,
  // AFL can receive more finegrained feedback via code-coverage over multiple
  // consecutive icmps.

  // if the BB branches on the given icmp then it is already fine and AFL should
  // get enough coverage feedback.
  auto term = bb->getTerminator();
  if (!term) {
    reportError("encountered BB without terminator", cmp_inst, M);
    return false;
  }
  if (auto branch = dyn_cast<BranchInst>(term)) {
    if (branch->isConditional()) {
      if (branch->getCondition() == cmp_inst) { return true; }
    }
  }

  // we split the BB right after the icmp instruction
  BasicBlock::iterator BI(cmp_inst);
  BI++;

  // errs() << "next I" << *BI << "\n";;

  BasicBlock *end_bb = bb->splitBasicBlock(BI);
  bb->getTerminator()->eraseFromParent();

  BasicBlock *cmp_true = BasicBlock::Create(C, "", end_bb->getParent(), end_bb);
  BasicBlock *cmp_false =
      BasicBlock::Create(C, "", end_bb->getParent(), end_bb);

  {
      // IntegerType *Int1Ty = IntegerType::getInt1Ty(C);
      // IRBuilder<> builder(end_bb);
      // PHINode *   PN = builder.CreatePHI(Int1Ty, 2, "");
      // PN->addIncoming(cmp_inst, cmp_true);
      // PN->addIncoming(cmp_inst, cmp_false);
      // cmp_inst->replaceAllUsesWith(PN);
  }

  {
    IRBuilder<> builder(cmp_inst);
    builder.CreateCondBr(cmp_inst, cmp_true, cmp_false);
  }

  {
    IRBuilder<> builder(cmp_true);
    builder.CreateBr(end_bb);
  }
  {
    IRBuilder<> builder(cmp_false);
    builder.CreateBr(end_bb);
  }
...
bool SplitComparesTransform::simplifyAndSplit(CmpInst *I, Module &M) {
  ...
  if (I->getPredicate() == CmpInst::ICMP_UGE ||
    ...
    if (!simplifyOrEqualsCompare(I, M, worklist)) {
      reportError("Failed to simplify UGE,SGE,ULE,SLE comparison", I, M);
    }
      ...
      for (auto &IN : BB) {
        if (auto CI = dyn_cast<CmpInst>(&IN)) {
          ...
          if (iTy1 && isa<IntegerType>(op1->getType())) {
            ...
            if (bitw >= target_bitwidth &&
                isSupportedBitWidth(bitw)) {  // && bitw <= MAX_COMP_SPLIT) {
